...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
  ...
  $type = $_POST['type'];
  $title = $_POST['title'];
  $principal_author = $_POST['principal_author'];
  ...
  $authoring_type = $_POST['authoring_type'];
  $publisher = $_POST['publisher'];
  $place_publication = $_POST['place_publication'];
  $url = $_POST['url'];
  $digital_identifier = $_POST['digital_identifier'];

  $sql = "INSERT INTO scw_ (type, title, principal_author, main_author, volume, no, year_published, edition, issn_isbn, class, level, publication_group, authoring_type, publisher, place_publication, url, digital_identifier, added_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  $stmt = mysqli_prepare($conn, $sql);

  if ($stmt) {
    mysqli_stmt_bind_param($stmt, "ssssssssssssssssss", $type, $title, $principal_author, $main_author, $volume, $no, $year_published, $edition, $issn_isbn, $class, $level, $publication_group, $authoring_type, $publisher, $place_publication, $url, $digital_identifier, $user_id);
    if (mysqli_stmt_execute($stmt)) {
      $user_id = $_SESSION['user_id'];
      $category = "scw";
      $description = "has added his/her scw.";
      $sql = "INSERT INTO `notification`(`user_id`, `category`, `description`) VALUES (?, ?, ?)";
      $stmt = mysqli_stmt_init($conn);
      if (!mysqli_stmt_prepare($stmt, $sql)) {
        header("location: ./scw.php?error");
        exit();
      }
        ...
        <div class="row page-titles">
          <ol class="breadcrumb">
            <li class="breadcrumb-item">
              <a href="tablescw.php">Scientific/Creative Works</a>
            </li>
            <li class="breadcrumb-item active">
              <a href="scw.php">Scientific/Creative Works Form</a>
            </li>
          </ol>
        </div>
        <div class="row">
          <div class="col s12">
            <div class="card">
              <div class="card-header">
                <h4 class="card-title">Scientific/Creative Works Form</h4>
              </div>
              <div class="card-body">
                <form method="post" action="" id="scw">
                  <div class="row">
                    <div class="col-lg-6 mb-3">
                      <label class="form-label">Type <span class="text-danger">*</span>
                      </label>
                      <select class="default-select form-control default-select wide" name="type" required>
                        <option value="">Choose Type</option>
                        <option>Book</option>
                        <option>Creative Work</option>
                        <option>Journal</option>
                        <option>Proceeding</option>
                        <option>Orginal Design</option>
                        <option>Techhinical Papers</option>
                    ...
                    <div class="col-lg-6 mb-3">
                      ...
                      <input class="form-control form-control-lg" rows="5" placeholder="Title" name="title" required></input>
                    </div>
                    <div class="col-lg-3 mb-3">
                      <label class="form-label">Principal Author? <span class="text-danger">*</span>
                      </label>
                      <select class="default-select form-control default-select wide" name="principal_author" required>
                        <option value="">Are you the principal Authors</option>
                        <option>Yes</option>
                        <option>No</option>
                      </select>
                    </div>
                    <div class="col-lg-9 mb-3">
                      <label class="form-label">Main/ Principal Author <span class="text-danger">*</span>
                      </label>
                      <input class="form-control form-control-lg" rows="5" placeholder="Last Name, First Name, Middle Name" name="main_author" required></input>
                    </div>
                    <div class="col-lg-4 mb-3">
                      <label class="form-label">Volume <span class="text-danger">*</span>
                      </label>
                      <input class="form-control form-control-lg" rows="5" placeholder="Volume" name="volume" required></input>
                    </div>
                    <div class="col-lg-4 mb-3">
                      <label class="form-label">No <span class="text-danger">*</span>
                      </label>
                      <input class="form-control form-control-lg" rows="5" placeholder="No." name="no" required></input>
                    </div>
                    <div class="col-lg-4 mb-3">
                      <label class="form-label">Year Published <span class="text-danger">*</span>
                      </label>
                      <input class="form-control form-control-lg" value="" type="date" name="year_published" required></input>
                    </div>
                    <div class="col-lg-6 mb-3">
                      <label class="form-label">Edition <span class="text-danger">*</span>
                      </label>
                      <input class="form-control form-control-lg" rows="5" placeholder="Edition" name="edition" required></input>
                    </div>
                    <div class="col-lg-6 mb-3">
                      <label class="form-label">ISSN/ISBN <span class="text-danger">*</span>
                      </label>
                      <input class="form-control form-control-lg" id="validationCustom04" rows="5" placeholder="ISSN/ISBN" name="issn_isbn" required></input>
                    </div>
                    <div class="col-lg-6 mb-3">
                      <label class="form-label">Class <span class="text-danger">*</span>
                      </label>
                      <select class="default-select form-control default-select wide" name="class" required>
                        <option value="">Select Class</option>
                        <option>Refereed</option>
                        <option>Non-Refereed</option>
                        <option>Refereed ISI</option>
                      </select>
                    </div>
                    <div class="col-lg-6 mb-3">
                      <label class="form-label">Level <span class="text-danger">*</span>
                      </label>
                      <select class="default-select form-control default-select wide" name="level" required>
                    ...
                    <div class="col-lg-6 mb-3">
                      <label class="form-label">Publication Group <span class="text-danger">*</span>
                      </label>
                      <select class="default-select form-control default-select wide" name="publication_group" required>
                        <option value="">Select Publication Group</option>
                        <option>R&D Papers in Scientific Journals</option>
                        <option>Technical Reports</option>
                        <option>Research Abstracts</option>
                        <option>Papers Presented in Conferences</option>
                        <option>Books</option>
                    ...
                    <div class="col-lg-6 mb-3">
                      <label class="form-label">Authoring Type <span class="text-danger">*</span>
                      </label>
                      <select class="default-select form-control default-select wide" name="authoring_type" required>
                        <option value="">Select Authoring Type</option>
                        <option>Sole-Author</option>
                        <option>Co-Author</option>
                        <option>Editor</option>
                        <option>Co-Editor</option>
                        <option>Main Author</option>
                        <option>Others</option>
                      </select>
                    </div>
                    <div class="col-lg-12 mb-3">
                      <label class="form-label">Publisher <span class="text-danger">*</span>
                      </label>
                      <input class="form-control form-control-lg" rows="5" placeholder="Publisher" name="publisher" required></input>
                    </div>
                    <div class="col-lg-12 mb-3">
                      <label class="form-label">Place of Publication <span class="text-danger">*</span>
                      </label>
                      <input class="form-control form-control-lg" rows="5" placeholder="Place of Publication" name="place_publication" required></input>
                    </div>
                    <div class="col-lg-12 mb-3">
                      <label class="form-label">URL of Publication <span class="text-danger">*</span>
                      </label>
                      <input class="form-control form-control-lg" rows="5" placeholder="Link to the online journal/publication" name="url" required></input>
                    </div>
                    <div class="col-lg-12 mb-3">
                      <label class="form-label">Digital Object Identifier <span class="text-danger">*</span>
                      </label>
                      <input class="form-control form-control-lg" rows="5" placeholder="Link to DOI" name="digital_identifier" required></input>
                    </div>
                    <div class="col-md-2" style="margin-top: 10px;">
                      <button type="submit" class="btn btn-primary mb-2">Submit</button>
                      <a href="tablescw.php" type="button" class="btn btn-danger light" data-bs-dismiss="modal">Close</a>
                    </div>
                  </div>
                </form>
              </div>
...
<script src="js/plugins-init/sweetalert.init.js"></script> -->
  ...
  <script>
    document.getElementById("scw").addEventListener("submit", function(e) {
    ...
    function showAlert() {
      swal({
        ...
        document.getElementById("scw").submit();
      });
