...
function addComment() {
  const commentText = document.getElementById(
    `commentText_${selectedId}`
  ).value;
  const category = document.getElementById(`comment-category-${selectedId}`).value;
  if (commentText) {
    var storeComment = new XMLHttpRequest();
    storeComment.open(
      "GET",
      "store.comment.php?id=" + selectedId + "&comment=" + commentText + "&category=" + category
    );
    storeComment.send();
...
function addReply(commentIndex, comment_index) {
  const reply_text = document.getElementById(
    `reply_text_${comment_index}`
  ).value;
...
function renderComments2(e) {
  ...
  function createCommentElement(comment, commentIndex) {
    ...
    var currentTimestampInHours = Math.floor(
      currentDateTime.getTime() / 1000 / 60 / 60
    );
    var mysqlTimestampInHours = Math.floor(
      mysqlDateTime.getTime() / 1000 / 60 / 60
    );
    var duration = currentTimestampInHours - mysqlTimestampInHours;
    var durationString;

    if (duration === 0) {
      currentTimestampInHours = Math.floor(
        currentDateTime.getTime() / 1000 / 60
      );
      mysqlTimestampInHours = Math.floor(mysqlDateTime.getTime() / 1000 / 60);
      duration = currentTimestampInHours - mysqlTimestampInHours;
      durationString = duration + "m";
    } else if (duration >= 24 * 7 * 4 * 12) {
      // If duration is greater than or equal to a year
      duration = Math.floor(duration / (24 * 7 * 4 * 12));
      durationString = duration + "y";
    } else if (duration >= 24 * 7 * 4) {
      // If duration is greater than or equal to a month
      duration = Math.floor(duration / (24 * 7 * 4));
      durationString = duration + "m";
    } else if (duration >= 24 * 7) {
      // If duration is greater than or equal to a week
      duration = Math.floor(duration / (24 * 7));
      durationString = duration + "w";
    } else if (duration >= 24) {
      // If duration is greater than or equal to a day
        ...
        replyElement.innerHTML = `
                        <div style="padding-bottom: 7px">
                            <div class="d-flex project-image" style="padding-left:50px">
                                ...
                                <div class="row">
                                    <div class="col">
                                        <div style="background:#d9d7d7; padding: 5px 10px; border-radius: 13px; padding-bottom:7px">${
                                          reply.text
                                        }</div>
                                    </div>
                                </div>
                        ...
                        <div class="row">
                            <div class="col">
                                <label style="border: 0; background:transparent; font-size: 11px; padding-right: 230px; padding-bottom: 5px">${
                                  currentTimestampInHours -
                                  mysqlTimestampInHours
                                }h</label>
                             </div>
                        </div>
                    `;
  ...
  commentsData.forEach((comment, index) => {
    if (comment["comments_id"] == e) {
      ...
      if(comment["comments_id"] == selectedId) {
        if(document.getElementById(`comment-category-${selectedId}`).value == "all") {
          commentElement = createCommentElement(comment, index);
        } else {
          if(comment["category"] == document.getElementById(`comment-category-${selectedId}`).value) {
            commentElement = createCommentElement(comment, index);
          } else {
            return;
          }
...
function renderComments() {
  ...
  function createCommentElement(comment, commentIndex) {
    ...
    commentElement.innerHTML = `
                <div class="comment">
                    <div>
                        <div class="d-flex project-image">
                            ...
                            <div class="row">
                                <div class="col">
                                    <div style="background:#d9d7d7; padding: 5px 10px; border-radius: 13px;">${
                                      comment.text
                                    }</div>
                                </div>
                            </div>
                ...
                <div class="row">
                    <div class="col">
                        <label style="border: 0; background:transparent; font-size: 11px; padding-left: 45px; padding-bottom: 5px">${
                          currentTimestampInHours - mysqlTimestampInHours
                        }h</label>
                        <button style="border: 0; background:transparent; font-size: 11px; padding-left: 13px; padding-bottom: 5px"onclick="openReplyBox(${
                          comment.index
                        })">Reply</button>
                    </div>
                </div>
                <div id="comment_${
                  comment.index
                }" style="padding-left: 40px; display: none; width: 100%; margin-bottom: 10px">
                    <textarea id="reply_text_${
                      comment.index
                    }" style="max-width: 340px; width: 100%; border-radius: 13px; padding-left: 10px" placeholder="Write your reply..."required></textarea>
                    <button class="btn btn-primary" style="margin-left: 5px; margin: 5px" onclick="addReply(${commentIndex}, ${
      comment.index
    })">Send</button>
                </div>
                `;
        ...
        replyElement.innerHTML = `
                        <div style="padding-bottom: 7px">
                            <div class="d-flex project-image" style="padding-left:50px">
                                ...
                                <div class="row">
                                    <div class="col">
                                        <div style="background:#d9d7d7; padding: 5px 10px; border-radius: 13px; padding-bottom:7px max-width: 500px; max-height: 500px">${
                                          reply.text
                                        }</div>
                                    </div>
                                </div>
