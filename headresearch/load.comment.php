...
$sql = "SELECT rtc.id, rtc.research_topic_id, rtc.comment, rtc.date_sent, fu.title, fu.first_name, fu.last_name, fu.image_path, rtc.sender FROM `research_topic_comments` AS `rtc` INNER JOIN `faculty_user` AS `fu` ON `rtc`.`sender` = `fu`.`id` ORDER BY date_sent";
