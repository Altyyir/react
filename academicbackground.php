...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
  ...
  $thesis = $_POST['thesis'];
  $sponsor_academic = $_POST['sponsor_academic'] ? $_POST['sponsor_academic'] : null;
  $primary_sponsor = $_POST['primary_sponsor'] ? $_POST['primary_sponsor'] : null;
  $scholarship_start = $_POST['scholarship_start'] ? $_POST['scholarship_start'] : null;
  $scholarship_end = $_POST['scholarship_end'] ? $_POST['scholarship_end'] : null;
  $extension_start = $_POST['extension_start'] ? $_POST['extension_start'] : null;
  $extension_end = $_POST['extension_end'] ? $_POST['extension_end'] : null;
  $item_expenses = $_POST['item_expenses'] ? $_POST['item_expenses'] : null;
  $amount_approved = $_POST['amount_approved'] ? $_POST['amount_approved'] : null;
  $amount_released = $_POST['amount_released'] ? $_POST['amount_released'] : null;
  $date_released = $_POST['date_released'] ? $_POST['date_released'] : null;

  $sql = "INSERT INTO academic_background (`degree`, `major_field`, `sector`, `institution`, `status`, `year_start`, `year_end`, `added_by`, `thesis`, `sponsor_academic`, `primary_sponsor`, `scholarship_start`, `scholarship_end`, `extension_start`, `extension_end`, `item_expenses`, `amount_approved`, `amount_released`, `date_released`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  $stmt = mysqli_prepare($conn, $sql);

  mysqli_stmt_bind_param($stmt, "sssssssssssssssssss", $degree, $major_field, $sector, $institution, $status, $year_start, $year_end, $user_id, $thesis, $sponsor_academic, $primary_sponsor, $scholarship_start, $scholarship_end, $extension_start, $extension_end, $item_expenses, $amount_approved, $amount_released, $date_released);
  mysqli_stmt_execute($stmt);
  mysqli_stmt_close($stmt);

  // $user_id = $_SESSION['user_id'];
  // $category = "academic background";
  // $description = "has added his/her academic background.";
  // $sql = "INSERT INTO `notification`(`user_id`, `category`, `description`) VALUES (?, ?, ?)";
  // $stmt = mysqli_stmt_init($conn);
  // if (!mysqli_stmt_prepare($stmt, $sql)) {
  //   header("location: ./academicbackground.php?error");
  //   exit();
  // }
        ...
        <div class="row page-titles">
          <ol class="breadcrumb">
            <li class="breadcrumb-item">
              <a href="tableacademic.php">Academic Background</a>
            </li>
            <li class="breadcrumb-item active">
              <a href="academicbackground.php">Academic Background Form</a>
            </li>
          </ol>
        </div>
        <div class="row">
          <div class="col s12">
            <div class="card">
              <div class="card-header">
                <h4 class="card-title">Academic Background Form</h4>
              </div>
              <div class="card-body">
                <form method="post" action="#" id="academic_background">
                  <div class="row">
                    <div class="col-lg-6 mb-3">
                      <label class="form-label ">Degree Earned (from highest to lowest) <span class="text-danger">*</span>
                      </label>
                      <input class="form-control form-control-lg" rows="5" placeholder="Degree Earned" name="degree" required></input>
                    </div>
                    <div class="col-lg-6 mb-3">
                      <label class="form-label ">Major Field <span class="text-danger">*</span>
                      </label>
                      <input class="form-control form-control-lg" rows="5" placeholder="Major Field" name="major_field" required></input>
                    </div>
                    <div class="col-lg-6 mb-3">
                      <label class="form-label ">Sector <span class="text-danger">*</span>
                      </label>
                      <input class="form-control form-control-lg" rows="5" placeholder="Sector" name="sector" required></input>
                    </div>
                    <div class="col-lg-6 mb-3">
                      <label class="form-label ">Learning Institution <span class="text-danger">*</span>
                      </label>
                      <input class="form-control form-control-lg" id="validationCustom04" rows="5" placeholder="Learning Institution" name="institution" required></input>
                    </div>
                    <div class="col-lg-4 mb-3">
                      <label class="form-label">Status <span class="text-danger">*</span>
                      </label>
                      <select id="inputState" class="default-select form-control default-select wide" name="status" required>
                        <option value="">Choose Status</option>
                        <option>Graduated</option>
                        <option>Ongoing</option>
                        <option>Dropped</option>
                        <option>Termminated</option>
                        <option>Others</option>
                      </select>
                    </div>
                    <div class="col-lg-4 mb-3">
                      <label class="form-label">Year Taken (Start)<span class="text-danger">*</span></label>
                      <input type="date" value="" class="form-control form-control-lg" name="year_start" required>
                    </div>
                    <div class="col-lg-4 mb-3">
                      <label class="form-label">(End) <span class="text-danger">*</span></label>
                      <input type="date" value="" class="form-control form-control-lg" name="year_end" required>
                    </div>
                    <div class="col-lg-12 mb-3">
                      <label class="form-label ">Thesis
                      </label>
                      <input class="form-control form-control-lg" id="validationCustom04" rows="5" placeholder="Thesis Title" name="thesis"></input>
                    </div>
                    <hr style="width: 97%; margin-left: 17px; margin-right: 20px; margin-top: 13px; padding: 1px">
                    <div class="row">
                      <div class="col-xl-12">
                        <div class="d-flex justify-content-between flex-wrap">
                          <div class="input-group contacts-search mb-8">
                            <h2 style="margin-left: 27px; margin-top: -3px; font-size: 19px; font-weight: 600; color: #000;">Scholarship</h2>
                    ...
                    <div class="col-lg-10 mb-3">
                      ...
                      <input class="form-control form-control-lg" id="validationCustom04" rows="5" placeholder="Sponsor" name="sponsor_academic"></input>
                    </div>
                    <div class="col-lg-2 mb-3">
                      <label class="form-label">Primary Sponsor
                      </label>
                      <select id="inputState" class="default-select form-control default-select wide" name="primary_sponsor">
                        <option>Choose Primary Sponsor</option>
                        <option>Yes</option>
                        <option>No</option>
                      </select>
                    </div>
                    <div class="col-lg-3 mb-3">
                      <label class="form-label">Scholarship Period (Start)</label>
                      <input type="date" value="" class="form-control form-control-lg" name="scholarship_start">
                    </div>
                    <div class="col-lg-3 mb-3">
                      <label class="form-label">(End)</label>
                      <input type="date" value="" class="form-control form-control-lg" name="scholarship_end">
                    </div>
                    <div class="col-lg-3 mb-3">
                      <label class="form-label">Extension Period (Start)</label>
                      <input type="date" value="" class="form-control form-control-lg" name="extension_start">
                    </div>
                    <div class="col-lg-3 mb-3">
                      <label class="form-label">(End)</label>
                      <input type="date" value="" class="form-control form-control-lg" name="extension_end">
                    </div>
                    <div class="col-lg-3 mb-3">
                      <label class="form-label">Item Expenses</label>
                      <input class="form-control form-control-lg" id="validationCustom04" rows="5" placeholder="Item Expenses" name="item_expenses"></input>
                    </div>
                    <div class="col-lg-3 mb-3">
                      <label class="form-label">Amount Approved</label>
                      <input type="number" value="" class="form-control form-control-lg" name="amount_approved">
                    </div>
                    <div class="col-lg-3 mb-3">
                      <label class="form-label">Amount Released</label>
                      <input type="number" value="" class="form-control form-control-lg" name="amount_released">
                    </div>
                    <div class="col-lg-3 mb-3">
                      <label class="form-label">Date Released</label>
                      <input type="date" value="" class="form-control form-control-lg" name="date_released">
                    </div>
                    <div class="col-md-2" style="margin-top: 10px;">
                      <button type="submit" class="btn btn-primary mb-2">Submit</button>
                      <a href="tableacademic.php" type="button" class="btn btn-danger light" data-bs-dismiss="modal">Close</a>
                    </div>
                  </div>
                </form>
              </div>
...
***********************************-->
  ...
  <script>
    ...
    document.getElementById("academic_background").addEventListener("submit", function(e) {
    ...
    function showAlert() {
      var form = document.getElementById("academic_background");
