    ...
    if(isset($_GET['id']))
        {
            $id = $_GET['id'];
                ...
                $query = "DELETE FROM ctc WHERE id='$id'";
        ...
        ***********************************-->
        <div class="header border-bottom">
            <div class="header-content">
              ...
              <div class="dashboard_bar">Capability Training Conducted
...
<!--                                <input type="text" class="form-control" placeholder="Search here...">
                                ...
                                <div class="mb-4">
                                    <a href="ctc.php" type="button" class="btn btn-primary btn-rounded fs-16"><i class="fas fa-plus me-3 scale4" style="color: #fff;"></i>Add More Capability Training Conducted</a>
                                  </div>
                            </div>
                        </div>
                    </div>              
                                ...
                                <table id="example" class="display">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="col-md-3" style="text-align: center;"><strong>Venue</strong></th>
                                            <th class="col-md-2" style="text-align: center;"><strong>Organizer</strong></th>
                                    ...
                                    <tbody>
                                        <?php 
                                                $query = "SELECT * FROM ctc WHERE added_by = {$user_id} ORDER BY `id` DESC";
                                        ...
                                        <tr>
                                           ...
                                           <td class="col-md-3"><div class="rounded-lg me-2" width="350px" alt="" style="text-align: left;"></div><?php echo $row["title_ctc"]?></td>
                                           <td class="col-md-3" style="text-align: center;"><?php echo $row["venue_ctc"]?></td>
                                            <td class="col-md-2" style="text-align: center;"><?php echo $row["organizer_ctc"]?></td>
                                            <?php
                                            $from_ctc = strtotime($row['from_ctc']);
                                            $from_ctc= date("M d, Y", $from_ctc);    
                                            $to_ctc = strtotime($row['to_ctc']);
                                            $to_ctc = date("M d, Y", $to_ctc);
                                        
                                            ?>
                                            <td class="col-md-2" style="text-align: center;" ><?php echo $from_ctc . " - " . $to_ctc; ?></td>
                                            <td class="col-md-1">
                                            <div class="d-flex justify-content-center" >
                                                <div class="d-flex">
                                                <a href="#" class="btn btn-dark shadow btn-xs sharp me-1" data-bs-toggle="modal" data-bs-target="#edit-modal-<?=$row['id']?>"><i class="fas fa-pencil-alt"></i></a>
                                                    <div id="edit-modal-<?=$row['id']?>" class="modal fade bd-example-modal-lg">
                                                        <div class="modal-dialog modal-lg ">
                                                            <div class="modal-content">
                                                              <form method="post" action="edit.ctc.php">
                                                                <input type="hidden" name="id" value="<?=$row['id']?>">
                                                                <div class="modal-header">
                                                                    <h5 class="modal-title" style="font-size: 23px">Update Capability Training Conducted</h5>
                                                                </div>
                                                                <div class="modal-body">
                                                                    <div class="row">
                                                                        <div class="col-md-12 mb-3">
                                                                            <label class="form-label">Theme/Title</label>
                                                                            <input class="form-control form-control-lg" rows="5" placeholder="Theme/Title" name="title_ctc" value="<?=$row['title_ctc']?>"></input>
                                                                        </div>
                                                                    </div>
                                                                    <div class="row">
                                                                        <div class="col-md-12 mb-3">
                                                                            <label class="form-label">Venue</label>
                                                                             <input class="form-control form-control-lg" id="validationCustom04" rows="5" placeholder="Venue" name="venue_ctc"value="<?=$row['venue_ctc']?>"></input>
                                                                        </div>

                                                                        <div class="col-md-6 mb-3">
                                                                            <label class="form-label">Organizer 
                                                                             </label>
                                                                             <select id="inputState" class="form-control" name="organizer_ctc" style="border-radius: 1rem; border: 0.0625rem solid #9DB2BF; color: #000;">
                                                                                <option>Choose Organizer</option>
                                                                                <option <?php if($row['organizer_ctc'] == 'Batangas State University'){echo "selected";}?>>Batangas State University</option>
                                                                                <option <?php if($row['organizer_ctc'] == 'Others'){echo "selected";}?>>Others</option>
                                                                              </select>
                                                                        </div>
                                                                        <div class="col-lg-6 mb-3">
                                                                           <label class="form-label">Level of Conference
                                                                            </label>
                                                                            <select id="inputState" class="form-control" name="conference_ctc" style="border-radius: 1rem; border: 0.0625rem solid #9DB2BF; color: #000;">
                                                                                <option>Choose Conference</option>
                                                                                <option <?php if($row['conference_ctc'] == 'Local'){echo "selected";}?>>Local</option>
                                                                                <option <?php if($row['conference_ctc'] == 'International'){echo "selected";}?>>International</option>
                                                                              </select>
                                                                          </div>
                                                                    </div>
                                                                    <div class="row">
                                                                        <div class="col-lg-6 mb-3">
                                                                            <label class="form-label">From
                                                                             ...
                                                                             <input type="date" class="form-control form-control-lg" name="from_ctc" value="<?=$row['from_ctc']?>">
                                                                        </div>
                                                                         <div class="col-lg-6 mb-3">
                                                                            <label class="form-label">To
                                                                            </label>
                                                                             <input type="date" class="form-control form-control-lg" name="to_ctc" value="<?=$row['to_ctc']?>">
                                                                          </div>
                                                                    </div>  
                                                                </div>
                                                ...
                                                <a onclick="deleteRecord(`<?=$row['id']?>`)" class="btn btn-dark shadow btn-xs sharp me-1"><i class="fas fa-trash"></i></a>
                                            </div>
        ...
        function deleteRecord(e) {
            ...
            .then((result) => {
              if(result) {
                ...
                deleteRecord.open("GET", "delete.record.php?table=ctc&id="+e); // yung pangalan lang ng table ang papalitan
                deleteRecord.send();
