...
while ($row = $result->fetch_assoc()) {
	if ($row['status'] == "Approved With Notice to Proceed" || $row['status'] == "Ongoing") {
		$id = $row['id'];
...
$query = "SELECT * FROM `research_topic` WHERE `added_by` = $user_id ORDER BY dateAdded DESC";
...
<body>
					...
					<div class="col-xl-12">
						<div class="d-flex justify-content-between align-items-center flex-wrap">
							...
							<div class="mb-4">
								...
								<a href="researchtopic.php" class="btn btn-primary btn-rounded fs-16"><i class="fas fa-plus me-3 scale4" style="color: #fff; font-size: 14px;"></i>Add Reseach Projects</a>
							</div>
						</div>
					</div>
				</div>
							...
							<div class="table-responsive">
								<table id="example5" class="display">
									...
									<tbody>
										...
										?>
											<tr>
												<td style="display: none;" class='center-align px-3'><strong><?= $row['end_date'] ?></td>
												<td class='col-md-3'><?php echo $row["project_title"] ?></td>
												<td class="col-md-2"><?php echo $row["partnership"] ?></td>
												<?php
												...
												<td class="col-md-1"><?php echo $start_date . " - " . $end_date; ?></td>
												<?php
												$namesArray = array();
												...
												<td class="center-align">
													<div class="d-flex">
														<div class="d-flex">
															<a href="updateresearchtopic.php?id=<?php echo $row['id'] ?>" class="btn btn-dark shadow btn-xs sharp me-1" title='Update'><i class="fas fa-pencil-alt"></i></a>
															<?php
															...
															?>
															<a href="" class="btn btn-dark shadow btn-xs sharp me-1" data-bs-toggle="modal" data-bs-target="#exampleModalCenter1" onclick="setImage(`<?= $row['approval_letter'] ?>`)" title='Approval Letter'><i class="fas fa-clipboard-check"></i></a>
																		...
																		<div class="modal-body" style="max-height: 280px;">
																			<div class="container3" style="margin-top: 0px;">
																				...
																				<div style="padding-left: 183px">
																					<a id="download-link" download="approval_letter.jpg">
															...
															<a href="pdf.php?id=<?php echo $row['id'] ?>" target="_blank" class="btn btn-dark shadow btn-xs sharp me-1" name="approve" value="approve"><i class="fas fa-cloud-download-alt" title='Project Proposal'></i></a>
														</div>
												</td>
											</tr>
										<?php
    ...
    ***********************************-->
	...
	<script src="js/plugins-init/delete.js"></script>
	...
	<script>
		function setImage(img) {
			...
			if (img) {
				// Set the image source and make the download button visible
				imageElement.src = "headresearch/" + img;
				downloadLink.href = "headresearch/" + img;
				downloadButton.style.display = 'block';
			} else {
				// Clear the image source and hide the download button when there is no image
				imageElement.src = '';
				downloadLink.href = '';
			...
			swal({
				...
				.then((result) => {
					if (result) {
						...
						deleteRecord.open("GET", "delete.record.php?table=research_topic&id=" + e); // yung pangalan lang ng table ang papalitan
						deleteRecord.send();
