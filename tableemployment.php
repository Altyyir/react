...
<!--                                <input type="text" class="form-control" placeholder="Search here...">
                                ...
                                <div class="mb-4">
                                    <a href="employment.php" type="button" class="btn btn-primary btn-rounded fs-16"><i class="fas fa-plus me-3 scale4" style="color: #fff;"></i>Add Employment</a>
                                  </div>
                            </div>
                        </div>
                    </div>              
                                ...
                                <table id="example5" class="display">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="col-md-3"><strong>Agency</strong></th>
                                            <th class="col-md-2" style="text-align: center;"><strong>Plantilla Position</strong></th>
                                            <th class="col-md-2" style="text-align: center;"><strong>Status of Appointment</strong></th>
                                            <th class="col-md-2" style="text-align: center;"><strong>Date of Appointment</strong></th>
                                            <th class="col-md-2" style="text-align: center;"><strong>Action</strong></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php 
                                            $query = "SELECT * FROM employment WHERE added_by = {$user_id} ORDER BY `id` DESC";
                                       ...
                                       <tr>
                                           ...
                                           <td class="col-md-3"><div class="rounded-lg me-2" width="350px" alt="" style="text-align: left;"></div><?php echo $row["agency"]?></td>
                                          <td class="col-md-2" style="text-align: center"><?php echo $row["plantilla_position"]?></td>
                                            <td class="col-md-2" style="text-align: center;"><?php echo $row["status"]?></td>
                                            <?php
                                            $appointment_start = strtotime($row['appointment_start']);
                                            $appointment_start = date("M d, Y", $appointment_start);    
                                            $appointment_end = strtotime($row['appointment_end']);
                                            $appointment_end = date("M d, Y", $appointment_end);
                                        
                                            ?>
                                            <td class="col-md-2" style="text-align: center;" ><?php echo $appointment_start . " - " . $appointment_end; ?></td>
                                            <td class="col-md-2">
                                                    ...
                                                    <div id="edit-modal-<?=$row['id']?>" class="modal fade bd-example-modal-lg">
                                                        <div class="modal-dialog modal-lg ">
                                                            <div class="modal-content">
                                                                <form method="post" action="edit.employment.php">
                                                                    <input type="hidden" name="id" value="<?=$row['id']?>">
                                                                    <div class="modal-header">
                                                                        <h5 class="modal-title" style="font-size: 23px">Update Employment</h5>
                                                                    </div>
                                                                    <div class="modal-body">
                                                                        <div class="row">
                                                                            <div class="col-lg-12 mb-3">
                                                                              <label class="form-label ">Agency <span class="text-danger">*</span>
                                                                              </label>
                                                                               <input class="form-control form-control-lg" id="validationCustom04" rows="5" placeholder="Sector" name="agency" value="<?=$row['agency']?>"></input>
                                                                            </div>
                                                                            <div class="col-lg-12 mb-3">
                                                                              <label class="form-label ">Plantilla Position <span class="text-danger">*</span>
                                                                              </label>
                                                                               <input class="form-control form-control-lg" id="validationCustom04" rows="5" placeholder="Sector" name="plantilla_position" value="<?=$row['plantilla_position']?>"></input>
                                                                            </div>
                                                                            <div class="col-lg-6 mb-3">
                                                                              <label class="form-label">Status of Appointment <span class="text-danger">*</span>
                                                                              </label>
                                                                                <select id="inputState" class="default-select form-control default-select wide" style="border-radius: 1rem; border: 0.0625rem solid #9DB2BF; color: #000;" name="status" required>
                                                                                  <option>Choose Status</option>
                                                                                  <option <?php if($row['status'] == 'Permanent'){echo "selected";}?>>Permanent</option>
                                                                                  <option <?php if($row['status'] == 'Temporary'){echo "selected";}?>>Temporary</option>
                                                                                  <option <?php if($row['status'] == 'Contratual'){echo "selected";}?>>Contratual</option>
                                                                                  <option <?php if($row['status'] == 'Casual'){echo "selected";}?>>Casual</option>
                                                                                  <option <?php if($row['status'] == 'Emergency'){echo "selected";}?>>Emergency</option>
                                                                                  <option <?php if($row['status'] == 'Others'){echo "selected";}?>>Others</option>
                                                                                </select>
                                                                            </div>
                                                                            <div class="col-lg-6 mb-3">
                                                                              <label class="form-label">Date of Appointment (Start) <span class="text-danger">*</span></label>
                                                                                <input type="date" class="form-control form-control-lg" name="appointment_start" value="<?=$row['appointment_start']?>" required>
                                                                            </div>
                                                                            <div class="col-lg-6 mb-3">
                                                                              <label class="form-label">(End) <span class="text-danger">*</span></label>
                                                                                <input type="date" class="form-control form-control-lg" name="appointment_end" value="<?=$row['appointment_end']?>" required>
                                                                            </div>
                                                                            <div class="col-lg-6 mb-3">
                                                                              <label class="form-label">Monthly Salary <span class="text-danger">*</span></label>
                                                                                <input type="number" class="form-control form-control-lg" name="monthly_salary" value="<?=$row['monthly_salary']?>" required>
                                                                            </div>
                                                                        </div> 
                                                                    </div>
        ...
        function deleteRecord(e) {
            ...
            .then((result) => {
              if(result) {
                ...
                deleteRecord.open("GET", "delete.record.php?table=employment&id="+e); // yung pangalan lang ng table ang papalitan
                deleteRecord.send();
