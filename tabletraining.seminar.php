...
if (isset($_GET['id'])) {
  ...
  $query = "DELETE FROM seminar_training WHERE id='$id'";
        ...
        <nav class="navbar navbar-expand">
          <div class="collapse navbar-collapse justify-content-between">
            <div class="header-left">
              <div class="dashboard_bar">Seminar/Training
        ...
        <div class="row">
          <div class="col-xl-12">
            <div class="d-flex justify-content-between align-items-center flex-wrap">
              ...
              <div class="mb-4">
                <a href="seminar.training.php" type="button" class="btn btn-primary btn-rounded fs-16"><i class="fas fa-plus me-3 scale4" style="color: #fff;"></i>Add More Seminar/Training</a>
              </div>
            </div>
          </div>
        </div>
                ...
                <table id="example" class="display">
                  <thead>
                    <tr>
                      ...
                      <th class="col-md-3" style="text-align: center;"><strong>Organizer</strong></th>
                      <th class="col-md-3" style="text-align: center;"><strong>Inclusive Date</strong></th>
                      <th class="col-md-2" style="text-align: center;"><strong>Action</strong></th>
                    </tr>
                  </thead>
                  <tbody>
                    <?php
                    $query = "SELECT * FROM seminar_training WHERE added_by = {$user_id} ORDER BY `id` DESC";
                    $result = mysqli_query($conn, $query);
                    $count = 1;
                    while ($row = mysqli_fetch_array($result)) {
                    ?>
                      <tr>
                        <td style="display: none;" class='center-align px-3'><?=$row['to_seminar']?></td>
                        <td class="col-md-3"><?php echo $row["title_seminar"] ?></td>
                        <td class="col-md-3" style="text-align: center;"><?php echo $row["organizer_seminar"] ?></td>
                        <?php
                        $from_seminar = strtotime($row['from_seminar']);
                        $from_seminar = date("M d, Y", $from_seminar);
                        $to_seminar = strtotime($row['to_seminar']);
                        $to_seminar = date("M d, Y", $to_seminar);

                        ?>
                        <td class="col-md-3" style="text-align: center;"><?php echo $from_seminar . " - " . $to_seminar; ?></td>
                        <td class="col-md-2">
                              ...
                              <div id="edit-modal-<?= $row['id'] ?>" class="modal fade bd-example-modal-lg">
                                <div class="modal-dialog modal-lg ">
                                  <div class="modal-content">
                                    <form method="post" action="edit.seminar.php">
                                      <input type="hidden" name="id" value="<?= $row['id'] ?>">
                                      <div class="modal-header">
                                        <h5 class="modal-title" style="font-size: 23px">Update Seminar/Training</h5>
                                      </div>
                                      <div class="modal-body">
                                        <div class="row">
                                          <div class="col-md-12 mb-3">
                                            <label class="form-label">Theme/Title</label>
                                            <input class="form-control form-control-lg" rows="5" placeholder="Theme/Title" name="title_seminar" value="<?= $row['title_seminar'] ?>"> </input>
                                          </div>

                                          <div class="col-md-12 mb-3">
                                            <label class="form-label">Venue</label>
                                            <input class="form-control form-control-lg" id="validationCustom04" rows="5" placeholder="Venue" name="venue_seminar" value="<?= $row['venue_seminar'] ?>"></input>
                                          </div>
                                        </div>
                                        <div class="row">
                                          <div class="col-md-6 mb-3">
                                            ...
                                            <input class="form-control form-control-lg" id="validationCustom04" rows="5" placeholder="Organizer" name="organizer_seminar" value="<?= $row['organizer_seminar'] ?>"></input>
                                          </div>
                                          <div class="col-lg-6 mb-3">
                                            <label class="form-label">Level of Seminar/Training
                                            </label>
                                            <select id="inputState" class="default-select form-control default-select wide" name="conference_seminar" style="border-radius: 1rem; border: 0.0625rem solid #9DB2BF; color: #000;">
                                              <option>Choose Conference</option>
                                              <option <?php if ($row['conference_seminar'] == 'Local') {
                                                        echo "selected";
                                                      } ?>>Local</option>
                                              <option <?php if ($row['conference_seminar'] == 'International') {
                                                        echo "selected";
                                        ...
                                        <div class="row">
                                          <div class="col-lg-6 mb-3">
                                            ...
                                            <input type="date" class="form-control form-control-lg" name="from_seminar" value="<?= $row['from_seminar'] ?>">
                                          </div>
                                          <div class="col-lg-6 mb-3">
                                            <label class="form-label">To
                                            </label>
                                            <input type="date" class="form-control form-control-lg" name="to_seminar" value="<?= $row['to_seminar'] ?>">
                                          </div>
                                        </div>
                                      </div>
      ...
      swal({
        ...
        .then((result) => {
          if (result) {
            ...
            deleteRecord.open("GET", "delete.record.php?table=seminar_training&id=" + e); // yung pangalan lang ng table ang papalitan
            deleteRecord.send();
